...
@Injectable()
export class ShippingInfoDeactive implements CanDeactivate<ShippingInfoComponent> {
  constructor(private _modalService: NgbModal,
              private _userContext: UserContext) {
    // empty
  }

  public canDeactivate(component: ShippingInfoComponent,
    ...
    const preData = component.preShippingInfoData;
    const curData = component.frm.getRawValue();
    if (!preData || !curData) {
      return true;
    }
    preData.shipments.forEach((ship) => {
      delete ship['formRequires'];
    });
    curData.shipments.forEach((ship) => {
      delete ship['formRequires'];
    });
