...
@Component({
  ...
  templateUrl: 'fit-sample.template.html',
  styleUrls: [
    'fit-sample.style.scss'
  ]
})
export class FitSampleComponent implements OnInit,
                                            AfterViewInit,
                                            AfterViewChecked,
                                            OnDestroy {
  ...
  public validationMessages = {
    ...
    rejectedDateOnUtc: {
      required: 'Date Rejected is required.'
    },
    qtyReceived: {
    ...
    files: {
      required: 'Fit Sample files is required.'
    },
    approvalDateOnUtc: {
  ...
  constructor(private _activatedRoute: ActivatedRoute,
              ...
              private _fitSampleService: FitSampleService,
  ...
  public buildForm(): void {
    let controlConfig = {
      ...
      fitSampleRequired: new FormControl(null, [Validators.required]),
  ...
  public getFitSample(): void {
    this.tableLoading = true;
    this._fitSampleService.getFitSampleInfo(this.styleId)
      .subscribe((resp: ResponseMessage<any>) => {
        if (resp.status) {
          this.fitSampleInfoData = resp.data;
          this.fitSampleHistories = resp.data.histories;
          this.updateForm(this.fitSampleInfoData);
        } else {
  ...
  public openUploader(frm: FormGroup, type: number): void {
    const funcUpload = (fileList: any[]) => {
      ...
      modalRef.componentInstance.title = 'Fit Sample';
        ...
        if (res.status) {
          ...
          if (res.deletedList && res.deletedList.length) {
            ...
            if (currentTypeList.length === 0 && res.newList.length === 0) {
              isShownMsg = true;
              this._toastrService
                .success(`Fit Sample files removed successfully.`, 'Success');
            }
          ...
          if (res.newList && res.newList.length) {
            ...
            if (!currentTypeList.length) {
              isShownMsg = true;
              this._toastrService
                .success(`Fit Sample files uploaded successfully.`, 'Success');
            }
  ...
  public openViewFiles(historyId): void {
    ...
    this._owsTechpackService.getHistoryFiles(this.styleId,
      historyId, TechPackFileTypes.FitSampleDetail)
      .subscribe((resp: ResponseMessage<any>) => {
        if (resp.status) {
          ...
          modalRef.componentInstance.title = 'Fit Sample History';
          Object.assign(modalRef.componentInstance.uploadOptions, {
            id: '',
            isReadOnly: true,
            fileList
  ...
  public onSubmitForm(type: number): void {
    ...
    this.tableLoading = true;
    this._fitSampleService.updateFitSampleInfo(this.styleId, type, this.frm.value)
      .subscribe((resp: ResponseMessage<any>) => {
        if (resp.status) {
          this.fitSampleInfoData = resp.data;
          this.fitSampleHistories = resp.data.histories;
