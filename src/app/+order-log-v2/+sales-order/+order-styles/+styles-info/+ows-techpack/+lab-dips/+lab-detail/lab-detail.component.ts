...
@Component({
  ...
  templateUrl: 'lab-detail.template.html',
  styleUrls: [
    'lab-detail.style.scss'
  ]
})
export class LabDetailComponent implements OnInit,
  ...
  public validationMessages = {
    ...
    files: {
      required: 'Lab Dip files is required.'
    },
    approvalDateOnUtc: {
  ...
  constructor(private _activatedRoute: ActivatedRoute,
              ...
              private _labDetailService: LabDetailService,
  ...
  public getLab(): void {
    ...
    this._labDetailService.getLabInfo(this.styleId, this.labId)
      .subscribe((resp: ResponseMessage<any>) => {
        if (resp.status) {
          this.labInfoData = resp.data;
          this.labsHistory = resp.data.histories;
          this.updateForm(this.labInfoData);
        } else {
  ...
  public openUploader(frm: FormGroup, type: number): void {
    const funcUpload = (fileList: any[]) => {
      ...
      modalRef.componentInstance.title = 'Lab Dip';
        ...
        if (res.status) {
          ...
          if (res.deletedList && res.deletedList.length) {
            ...
            if (currentTypeList.length === 0 && res.newList.length === 0) {
              isShownMsg = true;
              this._toastrService
                .success(`Lab Dip files removed successfully.`, 'Success');
            }
          ...
          if (res.newList && res.newList.length) {
            ...
            if (!currentTypeList.length) {
              isShownMsg = true;
              this._toastrService
                .success(`Lab Dip files uploaded successfully.`, 'Success');
            }
  ...
  public openViewFiles(historyId): void {
    ...
    this._owsTechpackService.getHistoryFiles(this.styleId,
      historyId, TechPackFileTypes.LabDipDetail)
      .subscribe((resp: ResponseMessage<any>) => {
        if (resp.status) {
          ...
          modalRef.componentInstance.title = 'Lab Dip History';
          Object.assign(modalRef.componentInstance.uploadOptions, {
            id: '',
            isReadOnly: true,
            fileList
  ...
  public onSubmitForm(type: number): void {
    ...
    this.tableLoading = true;
    this._labDetailService.updateLabInfo(this.styleId, this.labId, type, this.frm.value)
      .subscribe((resp: ResponseMessage<any>) => {
        if (resp.status) {
          this._labDipsService.setUpdateLab(resp.data);
          this.labInfoData = resp.data;
          this.labsHistory = resp.data.histories;
          this.updateForm(this.labInfoData);
