...
@Component({
  ...
  selector: 'packing-validation',
  // Our list of styles in our component. We may add more to compose many styles together
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
  // Every Angular template is first compiled by the browser before Angular runs it's compiler
  templateUrl: 'packing-validation.template.html',
  styleUrls: [
    'packing-validation.style.scss'
  ]
})
export class PackingValidationComponent implements OnInit,
                                            AfterViewInit,
                                            AfterViewChecked,
                                            OnDestroy {
  ...
  public validationMessages = {
    ...
    qtyReceived: {
      required: 'Qty Received is required.'
    },
    approvalStatus: {
      required: 'Approval Status is required.'
    },
    files: {
      required: 'Packing Validation files is required.'
    },
    approvalDateOnUtc: {
  ...
  public packingValidationInfoData: any;
  ...
  public packingValidationHistories = [];
  ...
  constructor(private _activatedRoute: ActivatedRoute,
              ...
              private _packingValidationService: PackingValidationService,
  ...
  public buildForm(): void {
    let controlConfig = {
      ...
      packingValidationRequired: new FormControl(null),
      ...
      qtyReceived: new FormControl(null),
  ...
  public getPackingValidation(): void {
    this.tableLoading = true;
    this._packingValidationService.getPackingValidationInfo(this.styleId)
      .subscribe((resp: ResponseMessage<any>) => {
        if (resp.status) {
          this.packingValidationInfoData = resp.data;
          this.packingValidationHistories = resp.data.histories;
          this.updateForm(this.packingValidationInfoData);
        } else {
  ...
  public openUploader(frm: FormGroup, type: number): void {
    const funcUpload = (fileList: any[]) => {
      ...
      modalRef.componentInstance.title = 'Packing Validation';
        ...
        if (res.status) {
          ...
          if (res.deletedList && res.deletedList.length) {
            ...
            if (currentTypeList.length === 0 && res.newList.length === 0) {
              isShownMsg = true;
              this._toastrService
                .success(`Packing Validation files removed successfully.`, 'Success');
            }
          ...
          if (res.newList && res.newList.length) {
            ...
            if (!currentTypeList.length) {
              isShownMsg = true;
              this._toastrService
                .success(`Packing Validation files uploaded successfully.`, 'Success');
            }
  ...
  public openViewFiles(historyId): void {
    ...
    this._owsTechpackService.getHistoryFiles(this.styleId,
      historyId, TechPackFileTypes.PackingValidate)
      .subscribe((resp: ResponseMessage<any>) => {
        if (resp.status) {
          ...
          modalRef.componentInstance.title = 'Packing Validation History';
          Object.assign(modalRef.componentInstance.uploadOptions, {
            id: '',
            isReadOnly: true,
            fileList
  ...
  public onSubmitForm(type: number): void {
    ...
    this.tableLoading = true;
    this._packingValidationService.updatePackingValidationInfo(this.styleId, type, this.frm.value)
      .subscribe((resp: ResponseMessage<any>) => {
        if (resp.status) {
          this.packingValidationInfoData = resp.data;
          this.packingValidationHistories = resp.data.histories;
