...
import { FilerType } from './sample-main-filter.model';
import {
  BasicCsrInfo,
  BasicCustomerInfo
...
import { SampleColumnsType } from '../../sample-main.model';
...
@Component({
  ...
  selector: 'sample-main-filter',  // <forgot-password></forgot-password>
  // Our list of styles in our component. We may add more to compose many styles together
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
  // Every Angular template is first compiled by the browser before Angular runs it's compiler
  templateUrl: 'sample-main-filter.template.html',
  styleUrls: [
    'sample-main-filter.style.scss'
  ]
})
export class SampleMainFilterComponent implements OnInit, OnChanges, OnDestroy {
  ...
  public listColumnName = _.sortBy(SampleColumnsType, 'name');
  ...
  public ngOnInit(): void {
    ...
    // Config filter
    let filterStore = this._localStorageService.get('Sample_FilterModel') as any;
    if (filterStore) {
      if (this.frm) {
  ...
  public getCustomer(): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this._sampleMainService.getCustomer()
  ...
  public configNavTabs(): void {
    const activatedTab = this.tabs.find((i) => i.name === this.frm.get('customer').value);
    if (activatedTab) {
    ...
    if (this.tabs && this.tabs.length) {
      const outsourceCurrentCustomer = this._localStorageService
        .get('Sample_CurrentCustomer') as any;
      if (outsourceCurrentCustomer) {
        setTimeout(() => {
          this.customerScroll.scrollTo(outsourceCurrentCustomer.scrollX, 0);
        });
      }
    }
  }
  ...
  public switchTab(index: number): void {
    ...
    if (index !== 0) {
      this.frm.get('customer').patchValue(this.tabs[index].name);
    } else {
      this.frm.get('customer').patchValue('');
    }
    this._localStorageService.set('Sample_CurrentCustomer',
      {
  ...
  public getFilterParam(): string {
    let filterParam = [
      ...
      // {
      //   filterType: FilerType.ColsName.toString(),
      //   value: this.frm.get('colsName').value || ''
      // },
      // {
      //   filterType: FilerType.ColStatus.toString(),
      //   value: this.frm.get('colStatus').value ?
      //     this.frm.get('colStatus').value.map((i) => i.id).join(',') : ''
      // },
      {
  ...
  public filter(): void {
    ...
    this._localStorageService.set('Sample_FilterModel',
      this.getFilterModel());
    this.onFilter.emit({
      keyword: this.keyword,
      filter: this.getFilterParam(),
