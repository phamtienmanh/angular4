...
@Injectable()
export class OutsourceMainService {
  constructor(private http: ExtendedHttpService) {
    // empty
  }

  public getListOutsource(params: HttpParams): Observable<ResponseMessage<any>> {
    return this.http.get(`/api/v1/outsource`, {
      params
    });
  }
  ...
  public changeStatusColumn(orderId: number,
    ...
    return this.http
      .put(`/api/v2/orders/${orderId}/change?columnType=${columnType}`,
        model);
  }
  ...
  public getScheduleColumnsInfo(orderId: number,
    ...
    if (printLocationId) {
      params = new HttpParams()
        ...
        .set('printLocationId', printLocationId.toString());
    }
    if (neckLabelId) {
      params = new HttpParams()
        .set('type', type.toString())
        .set('neckLabelId', neckLabelId.toString());
    }
    if (orderDetailId) {
      params = new HttpParams()
        .set('type', type.toString())
    ...
    return this.http.get(`/api/v2/orders/${orderId}/schedule-column`,
      {params});
  }
