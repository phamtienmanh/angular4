...
@Component({
  selector: 'phys-sample-actual',
  encapsulation: ViewEncapsulation.None,
  templateUrl: 'phys-sample-actual.template.html',
  styleUrls: [
    // 'phys-sample-actual.style.scss'
  ]
})
export class PhysSampleActualComponent implements OnInit {
  @Input()
  public orderId;
  @Input()
  ...
  public formErrors = {
    physSampleActualDateDeliveredPpCarrierId: '',
    physSampleActualDateDeliveredPpTracking: '',
  ...
  constructor(public activeModal: NgbActiveModal,
              ...
              private _physSampleActualService: PhysSampleActualService,
  ...
  public buildForm(): void {
    let controlConfig = {
      ...
      physSampleActualDateDeliveredPpStatus: new FormControl(null),
      physSampleActualDateDeliveredPpCarrierId: new FormControl(null),
      physSampleActualDateDeliveredPpCarrierName: new FormControl(''),
      physSampleActualDateDeliveredPpTracking: new FormControl(''),
      physSampleActualDateDeliveredPp: new FormControl(''),
      ...
      formRequires: new FormControl({
        physSampleActualDateDeliveredPpStatus: {
          required: false
        },
        physSampleActualDateDeliveredPpCarrierId: {
          required: false
  ...
  public getSpecialRequireCase(frm: FormGroup, key: string): boolean {
    if (key === 'firstCase') {
      ...
      ].indexOf(+this.frm.get('physSampleActualDateDeliveredPpStatus').value) > -1;
  ...
  public setDateValue(): void {
    ...
    patchDateFunc('physSampleActualDateDeliveredPpOnUtc',
  ...
  public getCommonData(): void {
    ...
    if (this.projectId && this.productId) {
      this._physSampleActualService
        .getDataProjectColumn(this.projectId, this.productId, ColumnType.PhysSampleActualDate)
        .subscribe((resp: ResponseMessage<any>) => {
          if (resp.status) {
      ...
      this._physSampleActualService.getDataColumn(this.orderId, this.orderDetailId, this.type)
        .subscribe((resp: ResponseMessage<any>) => {
          if (resp.status) {
            this.updateForm(resp.data);
            this._storedData = resp.data;
      ...
      if (this.projectId && this.productId) {
        this._physSampleActualService
          .changeStatusProjectColumn(this.projectId, this.productId,
            ColumnType.PhysSampleActualDate, modal)
          .subscribe((resp: ResponseMessage<any>) => {
            if (resp.status) {
