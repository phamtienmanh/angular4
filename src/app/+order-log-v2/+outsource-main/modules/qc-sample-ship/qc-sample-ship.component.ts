...
@Component({
  selector: 'qc-sample-ship',
  encapsulation: ViewEncapsulation.None,
  templateUrl: 'qc-sample-ship.template.html',
  styleUrls: [
    // 'qc-sample-ship.style.scss'
  ]
})
export class QcSampleShipComponent implements OnInit {
  @Input()
  public orderId;
  @Input()
  ...
  constructor(public activeModal: NgbActiveModal,
              ...
              private _qcSampleShipService: QcSampleShipService,
  ...
  public buildForm(): void {
    let controlConfig = {
      ...
      qcSampleDateCarrierId: new FormControl(null),
      qcSampleDateCarrierName: new FormControl(''),
      trackingNumber: new FormControl(''),
      dateSentToCustomer: new FormControl(''),
  ...
  public getCommonData(): void {
    ...
    if (this.projectId && this.productId) {
      this._qcSampleShipService
        .getDataProjectColumn(this.projectId, this.productId, ColumnType.QcSampleShipDate)
        .subscribe((resp: ResponseMessage<any>) => {
          if (resp.status) {
      ...
      this._qcSampleShipService.getDataColumn(this.orderId, this.orderDetailId, this.type)
        .subscribe((resp: ResponseMessage<any>) => {
          if (resp.status) {
            this.updateForm(resp.data);
            this._storedData = resp.data;
      ...
      if (this.projectId && this.productId) {
        this._qcSampleShipService
          .changeStatusProjectColumn(this.projectId, this.productId,
            ColumnType.QcSampleShipDate, modal)
          .subscribe((resp: ResponseMessage<any>) => {
            if (resp.status) {
