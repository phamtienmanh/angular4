...
@Component({
  ...
  templateUrl: 'eta-tsc.template.html',
  styleUrls: [
    // 'eta-tsc.style.scss'
  ]
})
export class EtaTscComponent implements OnInit {
  @Input()
  public orderId;
  @Input()
  ...
  public statusData = Status.filter((stt) => stt.name === 'Scheduled' || stt.name === 'Done');
  ...
  constructor(public activeModal: NgbActiveModal,
              ...
              private _etaTscService: EtaTscService,
  ...
  public buildForm(): void {
    let controlConfig = {
      ...
      etaTscDateStatus: new FormControl(null),
      etaTscDate: new FormControl(''),
      etaTscDateOnUtc: new FormControl('', [
        Validators.compose([
          ExtraValidators.conditional(
            (group) => this.getSpecialRequireCase(group, 'etaTscDateOnUtc'),
            Validators.required
          ),
      ...
      etaTscDateComment: new FormControl(''),
  ...
  public getSpecialRequireCase(frm: FormGroup, key: string): boolean {
    if (key === 'etaTscDateOnUtc') {
      let isRequired = [
        this.taskStatus.SCHEDULED,
        this.taskStatus.DONE
      ].indexOf(+this.frm.get('etaTscDateStatus').value) > -1;
  ...
  public getCommonData(): void {
    this._etaTscService.getDataColumn(this.orderId, this.orderDetailId, this.type)
      .subscribe((resp: ResponseMessage<any>) => {
        if (resp.status) {
          this.updateForm(resp.data);
          this._backupModel = this.frm.getRawValue();
  ...
  public onSelectItem(val, frm, valProp: string, formControlName: string): void {
    ...
    if (this._backupModel
      && this._backupModel.etaTscDateStatus === frm.get('etaTscDateStatus').value) {
      frm.patchValue(this._backupModel);
      return;
    }
    frm.get('etaTscDate').patchValue(null);
    frm.get('etaTscDateOnUtc').patchValue(null);
  }
