...
@Component({
  ...
  templateUrl: 'final-approval.template.html',
  styleUrls: [
    // 'final-approval.style.scss'
  ]
})
export class FinalApprovalComponent implements OnInit {
  ...
  public statusData = Status.filter((stt) => stt.name === 'Rejected'
    || stt.name === 'Approved w/ Changes' || stt.name === 'Approved'
    || stt.name === 'Dropped');

  public frm: FormGroup;
  public formErrors = {
    printApprovedDateRejectedOnUtc: '',
    printApprovedDateOnUtc: '',
    printApprovedDateDroppedOnUtc: '',
    printApprovedDateComment: ''
  };
  ...
  constructor(public activeModal: NgbActiveModal,
              ...
              private _finalApprovalService: FinalApprovalService,
  ...
  public buildForm(): void {
    let controlConfig = {
      ...
      printApprovalStatus: new FormControl(null),
      printApprovedDateRejected: new FormControl(''),
      printApprovedDateRejectedOnUtc: new FormControl('', [
        Validators.compose([
          ExtraValidators.conditional(
            (group) => this.getSpecialRequireCase(group, 'printApprovedDateRejectedOnUtc'),
            Validators.required
          ),
      ...
      printApprovedDateDropped: new FormControl(''),
      printApprovedDateDroppedOnUtc: new FormControl('', [
        Validators.compose([
          ExtraValidators.conditional(
            (group) => this.getSpecialRequireCase(group, 'printApprovedDateDroppedOnUtc'),
            Validators.required
          ),
          Validators.pattern('([0-9]{2})\/([0-9]{2})\/([0-9]{4})$' +
            '|^([0-9]{4})\-([0-9]{2})\-([0-9]{2})[T]([^a-z]+)'),
      ...
      printApprovedDateComment: new FormControl(''),
  ...
  public getSpecialRequireCase(frm: FormGroup, key: string): boolean {
    if (key === 'printApprovedDateOnUtc') {
      ...
      ].indexOf(+this.frm.get('printApprovalStatus').value) > -1;
      frm.get('formRequires').value[key].required = isRequired;
      return isRequired;
    } else if (key === 'printApprovedDateRejectedOnUtc') {
      let isRequired = [
        this.taskStatus.REJECTED
      ].indexOf(+this.frm.get('printApprovalStatus').value) > -1;
      frm.get('formRequires').value[key].required = isRequired;
      return isRequired;
    } else if (key === 'printApprovedDateDroppedOnUtc') {
      let isRequired = [
  ...
  public setDateValue(): void {
    ...
    patchDateFunc('printApprovedDateRejectedOnUtc', 'printApprovedDateRejected');
    patchDateFunc('printApprovedDateOnUtc', 'printApprovedDate');
    patchDateFunc('printApprovedDateDroppedOnUtc', 'printApprovedDateDropped');
  }
  ...
  public getCommonData(): void {
    if (this.projectId && this.productId) {
      this._finalApprovalService
        .getDataProjectColumn(this.projectId, this.productId,
          this.printLocationId, ColumnType.FinalApproval)
      ...
      this._finalApprovalService.getDataColumn(this.orderId, this.orderDetailId,
      ...
      if (this.projectId && this.productId) {
        this._finalApprovalService
          .changeStatusProjectColumn(this.projectId, this.productId,
            this.printLocationId, ColumnType.FinalApproval, modal)
          .subscribe((resp: ResponseMessage<any>) => {
            if (resp.status) {
        ...
        this._finalApprovalService.changeStatusColumn(this.orderId, this.orderDetailId,
