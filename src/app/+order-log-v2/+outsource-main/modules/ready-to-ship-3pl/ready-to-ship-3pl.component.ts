...
@Component({
  selector: 'ready-to-ship-3pl',
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: 'ready-to-ship-3pl.template.html',
  styleUrls: [
    // 'ready-to-ship-3pl.style.scss'
  ]
})
export class ReadyToShip3plComponent implements OnInit {
  @Input()
  public orderId;
  @Input()
  ...
  constructor(public activeModal: NgbActiveModal,
              ...
              private _readyToShip3plService: ReadyToShip3plService,
  ...
  public updateForm(data: any): void {
    if (data && data.length) {
      ...
      this.listUpdateReadyToShip.controls.forEach((shipment) => {
        ...
        if (shipment.get('applyToStyles').value && shipment.get('applyToStyles').value
  ...
  public getCommonData(): void {
    ...
    if (this.projectId && this.productId) {
      this._readyToShip3plService
        .getDataProjectColumn(this.projectId, this.productId)
        .subscribe((resp: ResponseMessage<any>) => {
          if (resp.status) {
            this.frm.get('status').patchValue(resp.data.status);
            this.updateForm(resp.data.listUpdateReadyToShip);
            this.originData.listUpdateReadyToShip
              = JSON.parse(JSON.stringify(resp.data.listUpdateReadyToShip));
      ...
      this._readyToShip3plService.getListReadyToShip(this.orderId, this.orderDetailId)
        .subscribe((resp: ResponseMessage<BasicGeneralInfo[]>) => {
          if (resp.status) {
            resp.data.forEach((i: any) => {
              if (!i.applyToStyles || !i.applyToStyles.length) {
                i.applyToStyles = [this.orderDetailId];
              }
            });
  ...
  public getLabelString(style): string {
    let leftLabel = '';
    if (style.partnerStyleName) {
      leftLabel += `(${style.partnerStyleName}) `;
    }
    leftLabel += `${style.styleName ? style.styleName : ''}${style.styleName
    && style.vendorStyleName ? ' - ' : ''}${style.vendorStyleName ? style.vendorStyleName : ''}`;
    // ------------------------------------------
  ...
  public onSubmitForm(): void {
    if (this.checkFormValid() && this.leadEtaComponent.checkFormValid()) {
      ...
      const formValue = Object.assign({}, this.frm.getRawValue());
      if (!this.frm.get('status').value || this.frm.get('status').value === this.taskStatus.BLANK) {
        formValue['listUpdateReadyToShip'] = [];
      }
      // is data change
      this._outsourceMainService
      ...
      if (this.projectId && this.productId) {
        this._readyToShip3plService
          .changeStatusProjectColumn(this.projectId, this.productId, modal)
          .subscribe((resp: ResponseMessage<any>) => {
            if (resp.status) {
