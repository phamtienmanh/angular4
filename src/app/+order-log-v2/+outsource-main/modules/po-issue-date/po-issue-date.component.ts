...
@Component({
  selector: 'po-issue-date',
  encapsulation: ViewEncapsulation.None,
  templateUrl: 'po-issue-date.template.html',
  styleUrls: [
    // 'po-issue-date.styles.scss'
  ]
})
export class PoIssueDateComponent implements OnInit {
  @Input()
  public orderId;
  @Input()
  ...
  public statusData = Status.filter((stt) => stt.name === 'Pending Approval'
    || stt.name === 'Rejected' || stt.name === 'Approved');
  ...
  constructor(public activeModal: NgbActiveModal,
              ...
              private _poIssueDateService: PoIssueDateService,
  ...
  public buildForm(): void {
    let controlConfig = {
      ...
      cutTicketStatus: new FormControl(null),
      cutTicketNumber: new FormControl('', Validators.compose([
          ExtraValidators.conditional(
            (group) => this.getSpecialRequireCase(group, 'cutTicketNumber'),
            Validators.required
          )
        ])
      ),
      cutTicketCreated: new FormControl(''),
      cutTicketCreatedOnUtc: new FormControl('', Validators.compose([
          ExtraValidators.conditional(
            (group) => this.getSpecialRequireCase(group, 'cutTicketCreatedOnUtc'),
            Validators.required
          ),
  ...
  public getSpecialRequireCase(frm: FormGroup, key: string): boolean {
    if (key === 'cutTicketNumber' || key === 'cutTicketCreatedOnUtc') {
      let isRequired = [
        this.taskStatus.PENDINGAPPROVAL,
        this.taskStatus.REJECTED,
        this.taskStatus.APPROVED
      ].indexOf(+this.frm.get('cutTicketStatus').value) > -1;
  ...
  public getCommonData(): void {
    this._poIssueDateService.getDataColumn(this.orderId, this.orderDetailId, this.type)
