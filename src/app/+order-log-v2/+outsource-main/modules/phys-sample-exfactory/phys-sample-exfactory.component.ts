...
@Component({
  selector: 'phys-sample-exfactory',
  encapsulation: ViewEncapsulation.None,
  templateUrl: 'phys-sample-exfactory.template.html',
  styleUrls: [
    // 'phys-sample-exfactory.style.scss'
  ]
})
export class PhysSampleExfactoryComponent implements OnInit {
  @Input()
  public orderId;
  @Input()
  ...
  public formErrors = {
    physSampleExFactoryDatePpCarrierId: '',
    physSampleExFactoryDatePpTracking: '',
  ...
  constructor(public activeModal: NgbActiveModal,
              ...
              private _physSampleExfactoryService: PhysSampleExfactoryService,
  ...
  public buildForm(): void {
    let controlConfig = {
      ...
      physSampleExFactoryDatePpStatus: new FormControl(null),
      physSampleExFactoryDatePpCarrierId: new FormControl(null),
      physSampleExFactoryDatePpCarrierName: new FormControl(''),
      physSampleExFactoryDatePpTracking: new FormControl(''),
      ...
      formRequires: new FormControl({
        physSampleExFactoryDatePpStatus: {
          required: false
        },
        physSampleExFactoryDatePpCarrierId: {
          required: false
  ...
  public getSpecialRequireCase(frm: FormGroup, key: string): boolean {
    if (key === 'firstCase') {
      ...
      ].indexOf(+this.frm.get('physSampleExFactoryDatePpStatus').value) > -1;
  ...
  public getCommonData(): void {
    ...
    if (this.projectId && this.productId) {
      this._physSampleExfactoryService
        .getDataProjectColumn(this.projectId, this.productId, ColumnType.PhysSampleExFactoryDate)
        .subscribe((resp: ResponseMessage<any>) => {
          if (resp.status) {
      ...
      this._physSampleExfactoryService.getDataColumn(this.orderId, this.orderDetailId, this.type)
        .subscribe((resp: ResponseMessage<any>) => {
          if (resp.status) {
            this.updateForm(resp.data);
            this._storedData = resp.data;
      ...
      if (this.projectId && this.productId) {
        this._physSampleExfactoryService
          .changeStatusProjectColumn(this.projectId, this.productId,
            ColumnType.PhysSampleExFactoryDate, modal)
          .subscribe((resp: ResponseMessage<any>) => {
            if (resp.status) {
