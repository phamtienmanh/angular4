...
@Component({
  selector: 'x-factory-date',
  encapsulation: ViewEncapsulation.None,
  templateUrl: 'x-factory-date.template.html',
  styleUrls: [
    // 'x-factory-date.style.scss'
  ]
})
export class XFactoryDateComponent implements OnInit {
  @Input()
  public orderId;
  @Input()
  ...
  constructor(public activeModal: NgbActiveModal,
              ...
              private _xFactoryDateService: XFactoryDateService,
  ...
  public buildForm(): void {
    let controlConfig = {
      ...
      exFactoryDateStatus: new FormControl(null),
      exFactoryDate: new FormControl(''),
      exFactoryDateOnUtc: new FormControl('', [
        Validators.compose([
          ExtraValidators.conditional(
            (group) => this.getSpecialRequireCase(group, 'exFactoryDateOnUtc'),
            Validators.required
          ),
  ...
  public getSpecialRequireCase(frm: FormGroup, key: string): boolean {
    if (key === 'exFactoryDateOnUtc') {
      let isRequired = [
        this.taskStatus.DONE
      ].indexOf(+this.frm.get('exFactoryDateStatus').value) > -1;
  ...
  public getCommonData(): void {
    if (this.projectId && this.productId) {
      this._xFactoryDateService
        .getDataProjectColumn(this.projectId, this.productId, ColumnType.XFactoryDate)
      ...
      this._xFactoryDateService.getDataColumn(this.orderId, this.orderDetailId, this.type)
        .subscribe((resp: ResponseMessage<any>) => {
          if (resp.status) {
            this.updateForm(resp.data);
            this._storedData = resp.data;
