...
import { FilerType } from './import-main-filter.model';
import {
  BasicCsrInfo,
  BasicCustomerInfo
...
@Component({
  ...
  selector: 'import-main-filter',
  // Our list of styles in our component. We may add more to compose many styles together
  encapsulation: ViewEncapsulation.None,
  changeDetection: ChangeDetectionStrategy.OnPush,
  // Every Angular template is first compiled by the browser before Angular runs it's compiler
  templateUrl: 'import-main-filter.template.html',
  styleUrls: [
    'import-main-filter.style.scss'
  ]
})
export class ImportMainFilterComponent implements OnInit, OnChanges, OnDestroy {
  ...
  public listColumnName = _.sortBy(ColumnsTypeImport, 'name');
  ...
  public ngOnInit(): void {
    ...
    // Config filter
    let filterStore = this._localStorageService.get('Import_FilterModel') as any;
    if (filterStore) {
      if (this.frm) {
  ...
  public getCustomerByType(): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this._commonService.getImportCustomersListByType(this.typeTab)
  ...
  public configNavTabs(): void {
    ...
    if (this.tabs && this.tabs.length) {
      const importCurrentCustomer = this._localStorageService
        .get('Import_CurrentCustomer') as any;
      if (importCurrentCustomer) {
        setTimeout(() => {
          this.customerScroll.scrollTo(importCurrentCustomer.scrollX, 0);
        });
      }
    }
  }
  ...
  public switchTab(index: number): void {
    ...
    this._localStorageService.set('Import_CurrentCustomer',
      {
  ...
  public filter(): void {
    ...
    this._localStorageService.set('Import_FilterModel',
      this.getFilterModel());
    this.onFilter.emit({
      keyword: this.keyword,
      filter: this.getFilterParam(),
