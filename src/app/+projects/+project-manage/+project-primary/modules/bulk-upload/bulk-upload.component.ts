...
import { ProjectPrimaryService } from '../../project-primary.service';

@Component({
  selector: 'bulk-upload',
  encapsulation: ViewEncapsulation.None,
  templateUrl: 'bulk-upload.template.html',
  styleUrls: [
    // 'bulk-upload.style.scss'
  ]
})
export class BulkUploadComponent implements OnInit {
  ...
  public buildForm(): void {
    let controlConfig = {
      ...
      bulkUploadDate: new FormControl(''),
      bulkUploadDateOnUtc: new FormControl('', [
        Validators.compose([
          ExtraValidators.conditional(
            (group) => this.getSpecialRequireCase(group, 'bulkUploadDateOnUtc'),
            Validators.required
          ),
      ...
      bulkUploadComments: new FormControl(''),
  ...
  public getSpecialRequireCase(frm: FormGroup, key: string): boolean {
    if (key === 'bulkUploadDateOnUtc') {
      let isRequired = [
        this.taskStatus.DONE
      ].indexOf(+this.frm.get('bulkUploadStatus').value) > -1;
  ...
  public setDateValue(): void {
    ...
    patchDateFunc('bulkUploadDateOnUtc', 'bulkUploadDate');
  }
  ...
  public openUploader(frm: FormGroup): void {
    const funcUpload = (fileList: any[]) => {
      ...
      modalRef.componentInstance.title = 'Bulk Upload';
      Object.assign(modalRef.componentInstance.uploadOptions, {
        uploadType: this.uploadedType.BulkUpload,
          ...
          if (res.newList && res.newList.length) {
            this._projectPrimaryService
              .uploadFile(this.projectId, this.productId,
                ColumnType.BulkUpload, res.newList)
              .subscribe((resp: ResponseMessage<any[]>) => {
                if (resp.status) {
                  if (resp.data.length) {
                    if (currentFiles.length) {
                      this._toastrService
                        .success(`Bulk Upload file(s) updated successfully.`,
                          'Success');
                    } else {
                      this._toastrService
                        .success(`Bulk Upload file(s) uploaded successfully.`,
                          'Success');
                    }
                    isShownMsg = true;
                    currentFiles = [
                ...
                if (resp.status) {
                  ...
                  if (!isShownMsg) {
                    if (currentFiles.length === 0 && res.newList.length === 0) {
                      this._toastrService
                        .success(`Bulk Upload file(s) removed successfully.`,
                          'Success');
                    } else {
                      this._toastrService
                        .success(`Bulk Upload file(s) updated successfully.`, 'Success');
                    }
                    isShownMsg = true;
                  }
                } else {
          ...
          if (res.updateList && res.updateList.length) {
            this._projectPrimaryService
              .updateFile(this.projectId, this.productId,
                ColumnType.BulkUpload, res.updateList)
              .subscribe((resp: BasicResponse) => {
                if (resp.status) {
                  if (!isShownMsg) {
                    this._toastrService
  ...
  public getCommonData(): void {
    this._projectPrimaryService
      .getFiles(this.projectId, this.productId, ColumnType.BulkUpload)
      .subscribe((resp: ResponseMessage<any>) => {
        if (resp.status) {
    ...
    this._bulkUploadService
      .getDataColumn(this.projectId, this.productId, ColumnType.BulkUpload)
      .subscribe((resp: ResponseMessage<any>) => {
        if (resp.status) {
  ...
  public onSubmitForm(): void {
    if (this.checkFormValid() && this.leadEtaComponent.checkFormValid()) {
      ...
      this._bulkUploadService
        .changeStatusColumn(this.projectId, this.productId, ColumnType.BulkUpload, modal)
        .subscribe((resp: ResponseMessage<any>) => {
          if (resp.status) {
