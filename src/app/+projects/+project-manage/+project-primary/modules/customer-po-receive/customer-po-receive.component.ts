...
@Component({
  selector: 'customer-po-receive',
  encapsulation: ViewEncapsulation.None,
  templateUrl: 'customer-po-receive.template.html',
  styleUrls: [
    // 'customer-po-receive.style.scss'
  ]
})
export class CustomerPoReceiveComponent implements OnInit {
  @Input()
  ...
  constructor(public activeModal: NgbActiveModal,
              ...
              private _customerPoReceiveService: CustomerPoReceiveService,
  ...
  public buildForm(): void {
    let controlConfig = {
      ...
      customerPoReceiveDateStatus: new FormControl(null),
      customerPoReceiveDate: new FormControl(''),
      customerPoReceiveDateOnUtc: new FormControl('', [
        Validators.compose([
          ExtraValidators.conditional(
            (group) => this.getSpecialRequireCase(group, 'customerPoReceiveDateOnUtc'),
            Validators.required
          ),
      ...
      customerPoReceiveDateComments: new FormControl(''),
      files: new FormControl([]),
      formRequires: new FormControl({
        customerPoReceiveDateStatus: {
          required: false
        },
        customerPoReceiveDateOnUtc: {
          required: false
  ...
  public getSpecialRequireCase(frm: FormGroup, key: string): boolean {
    if (key === 'customerPoReceiveDateOnUtc') {
      let isRequired = [
        this.taskStatus.DONE
      ].indexOf(+this.frm.get('customerPoReceiveDateStatus').value) > -1;
  ...
  public setDateValue(): void {
    ...
    patchDateFunc('customerPoReceiveDateOnUtc', 'customerPoReceiveDate');
  }
  ...
  public openUploader(frm: FormGroup, type: number): void {
    const funcUpload = (fileList: any[]) => {
      ...
      modalRef.componentInstance.title = 'Customer PO Receive Date';
      Object.assign(modalRef.componentInstance.uploadOptions, {
        uploadType: this.uploadedType.CustomerPoReceiveDate,
          ...
          if (res.newList && res.newList.length) {
            this._projectPrimaryService
              .uploadFile(this.projectId, this.productId,
                ColumnType.CustomerPoReceiveDate, res.newList)
              .subscribe((resp: ResponseMessage<any[]>) => {
                if (resp.status) {
                  if (resp.data.length) {
                    if (currentFiles.length) {
                      this._toastrService
                        .success(`Customer PO Receive Date file(s) updated successfully.`,
                          'Success');
                    } else {
                      this._toastrService
                        .success(`Customer PO Receive Date file(s) uploaded successfully.`,
                          'Success');
                    }
                    isShownMsg = true;
                    currentFiles = [
                ...
                if (resp.status) {
                  ...
                  if (!isShownMsg) {
                    if (currentFiles.length === 0 && res.newList.length === 0) {
                      this._toastrService
                        .success(`Customer PO Receive Date file(s) removed successfully.`,
                          'Success');
                    } else {
                      this._toastrService
                        .success(`Customer PO Receive Date file(s) updated successfully.`,
          ...
          if (res.updateList && res.updateList.length) {
            this._projectPrimaryService
              .updateFile(this.projectId, this.productId,
                ColumnType.CustomerPoReceiveDate, res.updateList)
              .subscribe((resp: BasicResponse) => {
                if (resp.status) {
                  if (!isShownMsg) {
                    this._toastrService
                      .success(`Customer PO Receive Date file(s) updated successfully.`, 'Success');
                    isShownMsg = true;
                  }
                } else {
                  this._toastrService.error(resp.errorMessages, 'Error');
  ...
  public getCommonData(): void {
    this._projectPrimaryService
      .getFiles(this.projectId, this.productId, ColumnType.CustomerPoReceiveDate)
      .subscribe((resp: ResponseMessage<any>) => {
        if (resp.status) {
    ...
    this._customerPoReceiveService
      .getDataColumn(this.projectId, this.productId, ColumnType.CustomerPoReceiveDate)
      .subscribe((resp: ResponseMessage<any>) => {
        if (resp.status) {
  ...
  public onSubmitForm(): void {
    if (this.checkFormValid() && this.leadEtaComponent.checkFormValid()) {
      ...
      this._customerPoReceiveService
        .changeStatusColumn(this.projectId, this.productId, ColumnType.CustomerPoReceiveDate, modal)
        .subscribe((resp: ResponseMessage<any>) => {
          if (resp.status) {
