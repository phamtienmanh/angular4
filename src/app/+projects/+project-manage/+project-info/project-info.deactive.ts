...
@Injectable()
export class ProjectInfoDeactive implements CanDeactivate<ProjectInfoComponent> {
  constructor(private _modalService: NgbModal,
              private _userContext: UserContext) {
    // empty
  }

  public canDeactivate(component: ProjectInfoComponent,
                       currentRoute: ActivatedRouteSnapshot,
                       currentState: RouterStateSnapshot,
                       nextState: RouterStateSnapshot) {
    if (currentState.url.match(/\/projects\/new-project\/project-info/)
      && nextState.url.match(/\/projects\/\d+\/project-info/)) {
      return true;
    }
    if (!this._userContext.currentUser.id) {
      return true;
    }
    const preData = component.preProjectInfoData;
