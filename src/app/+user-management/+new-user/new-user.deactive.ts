...
@Injectable()
export class NewUserDeactive implements CanDeactivate<NewUserComponent> {
  constructor(private _modalService: NgbModal,
              private _userContext: UserContext) {
    // empty
  }

  public canDeactivate(component: NewUserComponent,
                       currentRoute: ActivatedRouteSnapshot,
                       currentState: RouterStateSnapshot,
                       nextState: RouterStateSnapshot) {
    if (currentState.url.includes('user-management/new-user')
      && nextState.url.match(/\/user-management\/\d+/)) {
      return true;
    }
