...
@Injectable()
export class VendorsEditDeactive implements CanDeactivate<VendorsEditComponent> {
  constructor(private _modalService: NgbModal,
              private _userContext: UserContext) {
    // empty
  }

  public canDeactivate(component: VendorsEditComponent,
                       currentRoute: ActivatedRouteSnapshot,
                       currentState: RouterStateSnapshot,
                       nextState: RouterStateSnapshot) {
    if (currentState.url.includes('vendors-management/new')
      && nextState.url.match(/\/vendors-management\/\d+\/detail/)) {
      return true;
    }
    if (!this._userContext.currentUser.id) {
      return true;
    }
    const preData = component.preVendorEditData;
