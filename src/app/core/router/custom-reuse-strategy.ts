 ...
 * An ActivatedRouteSnapshot,
 * which is useful for determining whether or not you should attach a route
 * (see this.shouldAttach)
...
export class CustomReuseStrategy implements RouteReuseStrategy {
   ...
   * If the route should be stored,
   * I believe the boolean is indicating to a controller whether or not to fire this.store
  ...
  public store(route: ActivatedRouteSnapshot, handle: DetachedRouteHandle): void {
    ...
    // and the handle is stored under it so that you can only ever have one object stored
    // for a single path
  ...
  public shouldAttach(route: ActivatedRouteSnapshot): boolean {
    ...
    // this decides whether the route already stored should be rendered in place
    // of the requested route, and is the return value
    // at this point we already know that the paths match
    // because the storedResults key is the route.routeConfig.path
    ...
    if (canAttach) {
      ...
      let queryParamsMatch: boolean =
        this.compareObjects(route.queryParams, storedRoute.snapshot.queryParams);

      // console.log("deciding to attach...", route, "does it match?", storedRoute.snapshot,
   ...
   * which both objects have, not strict comparison (===)
   * @param base The base object which you would like to compare another object to
   * @param compare The object to compare to base
   * @returns boolean indicating whether
   * or not the objects have all the same properties and those properties are ==
   */
